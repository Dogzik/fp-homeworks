...
spec = do
  describe "eval evaluates expresseion represented by Expr class" $ do
    it "Const X is always X" $ property $
      \(x :: Int) -> eval (Const x) `shouldBe` Right x
    it "Add, Sub and Mul work as +, - and *" $ property $
